...
# ---- Connect and read directly from the feature group ----
...
print(f"After merging with targets: {len(df)} rows")

# ---- Feature/exclude logic ----
exclude_cols = ["time", "target_day1", "target_day2", "target_day3"]
feature_cols = [c for c in df.columns if c not in exclude_cols]
print(f"Using {len(feature_cols)} features")

# Save feature order for consistent use at prediction time
...
for target in targets:
    ...
    dump(ridge_final, f"models/ridge_{target}_final.joblib")
    dump(rf_final, f"models/rf_{target}_final.joblib")
    print(f"\nSaved models/ridge_{target}_final.joblib and models/rf_{target}_final.joblib")